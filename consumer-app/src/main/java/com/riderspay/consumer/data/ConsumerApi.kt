...
class ConsumerApi(private val db:FirebaseFirestore=FirebaseFirestore.getInstance(),private val fn:FirebaseFunctions=FirebaseFunctions.getInstance()){
    ...
    suspend fun book(standId:String,pickup:Point,drop:Point,quote:Quote):String {
        fun wire(p:Point)=mapOf("lat" to p.lat,"lng" to p.lng,"address" to p.address)
        val d=call("createRide",mapOf("standId" to standId,"pickup" to wire(pickup),"drop" to wire(drop)))
        return d["rideId"].toString()
    }
